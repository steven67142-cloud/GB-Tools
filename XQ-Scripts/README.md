...
截圖已遮蓋地圖上玩家名稱與快訊個資；公開前請再確認無憑證網址、token、社群密碼。

### 如何取得遊戲資格

1. 綁定 XQ 推薦碼 **`@GB`**
2. 執行任一 GB 受保護選股（建議外資買進；網址見腳本描述或執行訊息）
3. 開啟探吉島遊戲網址建立／登入角色
...
### [探吉島入社導引（掛既有選股）](./探吉島入社導引_v1.0/)

不另建假腳本。把遊戲網址加進既有 `@GB` 選股的「描述／Message」（建議先改外資買進），玩家綁碼執行即可看到連結。
